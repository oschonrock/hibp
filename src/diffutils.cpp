...
void run_diff(const std::filesystem::path& old_path,const std::filesystem::path& new_path, std::ostream& diff) {
  ...
  while (true) {
    ...
    if (std::next(diff_iter_old) != db_old.end() && deep_equals(*std::next(diff_iter_old), *diff_iter_new)) {
      throw std::runtime_error("Deletion from OLD. This shouldn't happen!");
    }
    // fine to dereference std::next(new)
    if (std::next(diff_iter_new) != db_new.end() && deep_equals(*diff_iter_old, *(diff_iter_new + 1))) {
      diff << fmt::format("I:{:08X}:{}\n", diff_iter_old - db_old.begin(),
                               diff_iter_new->to_string());
...
template void run_diff<hibp::pawned_pw_sha1>(const std::filesystem::path& old_path, const std::filesystem::path& new_path, std::ostream& diff);
template void run_diff<hibp::pawned_pw_ntlm>(const std::filesystem::path& old_path, const std::filesystem::path& new_path, std::ostream& diff);
